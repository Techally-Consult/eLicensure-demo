...
- `/apply` → Application wizard (new); `/apply/$id` → Application wizard (edit)
...
- **Wizard state (local):** ApplicationWizardPage: full WizardState; useAuth(); when route has `id` init from applicationToWizardState once; when search `type=renewal|variation|additional` set licenseType. Submit: updateApplication(id) or submitApplication({ ...payload, status: 'Submitted', applicantUserId: user?.id }).

## Mock Data Shapes

- **User:** id, email, name, role (UserRole).
- **Application:** id, licenseType, facilityName, status, lastUpdated, timeline; optional applicant, facility, facilityId, services, totalBeds, staffingHead, staffRows, infrastructureDescription, typeSpecific, **applicantUserId**, **assignedTo**, **inspection** (InspectionResult), **remark**. Status flow: Draft → Submitted → Assigned → Under Inspection → Inspection Submitted/Rejected → Approved or Rejected.
...
| Wizard | Tabs + Card, RadioGroup (license type), full form; create/edit; search param type pre-select |
...
- **Wizard:** One component for create/edit; id from route; search type for license pre-select; full payload on submit.
